cs_con_str_est <-
  function(x_i,y_i,w_i,beta,corstr,family){
    ...
    mat<-diag(w_i,nrow=length(w_i),ncol=length(w_i))
    ...
    # print(w_i)
    # print(std_resi)
    if(corstr=="ar1" & length(w_i) > 1){
      if (nrow(std_resi)-1 == 1){
        res=sum(diag(std_resi[1:(nrow(std_resi)-1),2:ncol(std_resi)],nrow=1,ncol=1),na.rm=T)
      }else{
